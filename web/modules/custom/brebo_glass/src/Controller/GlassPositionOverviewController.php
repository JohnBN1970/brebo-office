...
final class GlassPositionOverviewController extends ControllerBase {
  ...
  public function overview(): array {
    ...
    foreach($positions as $p){ $risk=$this->riskEvaluator->evaluate($p); $p['_risk']=$risk; $prioritized[]=$p; if($risk['level']==='kritiek')$critical++; elseif($risk['level']==='aandacht')$attention++; }
    ...
    foreach($prioritized as $p){
      $id=(int)$p['id'];$buildingId=(int)$p['building_nid'];$projectId=(int)($p['project_nid']??0);$state=(string)$p['technical_status'];$policy=$this->approvalPolicy->evaluate($p);$risk=$p['_risk'];
      ...
      $riskText=$risk['score']>0 ? strtoupper($risk['level']).' · '.implode('; ',$risk['reasons']) : $this->t('Normaal');
      $rows[]=[
        'risk'=>$riskText,
        ...
        'technical_check'=>$this->t(self::CHECK_LABELS[$p['technical_check_state']]??$p['technical_check_state']),'status'=>$this->t(self::STATUS_LABELS[$state]??$state),
        'signal'=>$signal,'approval'=>$approval,'calculation'=>$calculation,'procurement'=>$procurement,'mounting'=>$mounting,
      ];
    }

    $build['alerts']=['#theme'=>'item_list','#title'=>$this->t('Vandaag aandacht nodig'),'#items'=>[
      $this->t('Kritiek: @n',['@n'=>$critical]),$this->t('Aandacht: @n',['@n'=>$attention]),$this->t('Posities totaal: @n',['@n'=>$counts['all']??0]),
    ]];
    $build['summary']=['#theme'=>'item_list','#title'=>$this->t('Operationele glasstatus'),'#items'=>[
      $this->t('Vrijgegeven: @n',['@n'=>$counts['approved']??0]),$this->t('Besteld: @n',['@n'=>$counts['ordered']??0]),$this->t('Geleverd: @n',['@n'=>$counts['delivered']??0]),$this->t('Gemonteerd: @n',['@n'=>$counts['installed']??0]),
    ]];
