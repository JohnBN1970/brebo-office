...
function brebo_calculation_post_update_add_object_source_traceability(&$sandbox = NULL): string {
  ...
  if (!$schema->tableExists($table)) return 'BREBO Calculation row domain is not installed; no source traceability fields added.';
  $fields = ['source_domain' => ['type' => 'varchar_ascii', 'length' => 64, 'not null' => FALSE], 'source_reference' => ['type' => 'varchar', 'length' => 255, 'not null' => FALSE]];
  $added = [];
  foreach ($fields as $name => $definition) if (!$schema->fieldExists($table, $name)) { $schema->addField($table, $name, $definition); $added[] = $name; }
  if (!$schema->indexExists($table, 'object_source')) $schema->addIndex($table, 'object_source', ['source_domain', 'source_reference']);
  return $added ? 'BREBO Calculation object source traceability added: ' . implode(', ', $added) . '.' : 'BREBO Calculation object source traceability already exists.';
}
...
function brebo_calculation_post_update_create_norm_library(&$sandbox = NULL): string {
  ...
  $schema->createTable($table, [
    ...
    'fields' => [
      ...
      'norm_key' => ['type' => 'varchar_ascii', 'length' => 64, 'not null' => TRUE],
      'label' => ['type' => 'varchar', 'length' => 255, 'not null' => TRUE],
      'value' => ['type' => 'numeric', 'precision' => 18, 'scale' => 6, 'not null' => TRUE],
      'unit' => ['type' => 'varchar', 'length' => 32, 'not null' => FALSE],
      'conditions_json' => ['type' => 'text', 'size' => 'big', 'not null' => FALSE],
      'priority' => ['type' => 'int', 'not null' => TRUE, 'default' => 0],
      'active' => ['type' => 'int', 'size' => 'tiny', 'unsigned' => TRUE, 'not null' => TRUE, 'default' => 1],
      'source' => ['type' => 'varchar', 'length' => 255, 'not null' => FALSE],
      'changed' => ['type' => 'int', 'unsigned' => TRUE, 'not null' => TRUE, 'default' => 0],
    ],
    'primary key' => ['id'],
    'indexes' => ['domain_key_active' => ['domain', 'norm_key', 'active'], 'priority' => ['priority']],
  ]);
...
function brebo_calculation_post_update_create_norm_observation_feedback(&$sandbox = NULL): string {
  ...
  $schema->createTable($table, [
    'description' => 'Actual observations used to evaluate and improve BREBO norms without duplicating source administration.',
    'fields' => [
      'id' => ['type' => 'serial', 'not null' => TRUE],
      'domain' => ['type' => 'varchar_ascii', 'length' => 64, 'not null' => TRUE],
      'norm_key' => ['type' => 'varchar_ascii', 'length' => 64, 'not null' => TRUE],
      'planned_value' => ['type' => 'numeric', 'precision' => 18, 'scale' => 6, 'not null' => TRUE],
      'actual_value' => ['type' => 'numeric', 'precision' => 18, 'scale' => 6, 'not null' => TRUE],
      'unit' => ['type' => 'varchar', 'length' => 32, 'not null' => TRUE],
      'delta_value' => ['type' => 'numeric', 'precision' => 18, 'scale' => 6, 'not null' => TRUE],
      'delta_pct' => ['type' => 'numeric', 'precision' => 12, 'scale' => 4, 'not null' => FALSE],
      'source_domain' => ['type' => 'varchar_ascii', 'length' => 64, 'not null' => TRUE],
      'source_reference' => ['type' => 'varchar', 'length' => 255, 'not null' => TRUE],
      'project_id' => ['type' => 'int', 'unsigned' => TRUE, 'not null' => FALSE],
      'context_json' => ['type' => 'text', 'size' => 'big', 'not null' => FALSE],
      'created' => ['type' => 'int', 'unsigned' => TRUE, 'not null' => TRUE],
    ],
    'primary key' => ['id'],
    'indexes' => [
      'norm_lookup' => ['domain', 'norm_key'],
      'source_lookup' => ['source_domain', 'source_reference'],
