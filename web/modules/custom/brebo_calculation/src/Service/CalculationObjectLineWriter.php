...
final class CalculationObjectLineWriter {
  ...
  public function write(int $calculationId,string $version,string $paragraphKey,string $description,float $quantity,string $unit,array $unitCosts,string $sourceDomain,string $sourceReference,string $sourceChecksum,AccountInterface $account): int {
    ...
    try {
      ...
      $this->setIfPresent($line,'field_brebo_line_status','Niet beoordeeld');$this->setIfPresent($line,'field_brebo_line_type','Calculatieregel');$this->setIfPresent($line,'field_brebo_note_visibility','Intern');$this->setIfPresent($line,'field_brebo_line_note',sprintf('Objectbron %s:%s · checksum %s',$sourceDomain,$sourceReference,$sourceChecksum));
      ...
      $values=$costs+['source_domain'=>$sourceDomain,'source_reference'=>$sourceReference,'source_checksum'=>$sourceChecksum];
