...
final class CalculationNormFeedbackService {
  ...
  public function record(
    ...
    foreach ([$domain, $normKey, $unit, $sourceDomain, $sourceReference] as $required) {
      if (trim($required) === '') throw new \InvalidArgumentException('Normobservatie mist verplichte bron- of normgegevens.');
    }
    ...
    $deltaPct = $plannedValue > 0 ? ($delta / $plannedValue) * 100 : NULL;
    return (int) $this->database->insert('brebo_calculation_norm_observation')->fields([
      'domain' => $domain,
      'norm_key' => $normKey,
  ...
  public function summary(string $domain, string $normKey, int $minimumSamples = 3): array {
    ...
    $actualAverage = isset($row['actual_avg']) ? (float) $row['actual_avg'] : NULL;
    return [
      'samples' => $samples,
      'planned_avg' => isset($row['planned_avg']) ? (float) $row['planned_avg'] : NULL,
      'actual_avg' => $actualAverage,
      'delta_pct_avg' => isset($row['delta_pct_avg']) ? (float) $row['delta_pct_avg'] : NULL,
      'proposed_value' => $samples >= $minimumSamples ? $actualAverage : NULL,
    ];
  }
