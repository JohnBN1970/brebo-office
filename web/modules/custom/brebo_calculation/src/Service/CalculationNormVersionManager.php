...
/** Creates traceable replacement versions of calculation norms. */
final class CalculationNormVersionManager {
  ...
  public function createReplacement(int $normId,float $value,string $reason,AccountInterface $account,?int $embeddingId=NULL): int {
    ...
    try {
      ...
      return (int)$this->database->insert('brebo_calculation_norm')->fields([
        'domain'=>$current['domain'],'norm_key'=>$current['norm_key'],'label'=>$current['label'],'value'=>$value,'unit'=>$current['unit'],
        'conditions_json'=>$current['conditions_json'],'priority'=>$current['priority'],'active'=>1,'source'=>$source,'changed'=>time(),
      ])->execute();
