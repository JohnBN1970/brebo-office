...
final class SubcalculationOverviewForm extends FormBase {
  ...
  public function getFormId(): string {
    return 'brebo_calculation_subcalculation_overview_form';
  }
  ...
  public function buildForm(array $form, FormStateInterface $form_state, ?NodeInterface $node = NULL): array {
    ...
    $records = $this->database->select('brebo_calculation_subcalculation', 's')
      ->fields('s')
    ...
    foreach ($records as $id => $record) {
      $scopeCount = (int) $this->database->select('brebo_calculation_subcalculation_scope', 'ss')
        ->condition('subcalculation_id', (int) $id)
        ->countQuery()->execute()->fetchField();
      $applicationCount = (int) $this->database->select('brebo_calculation_subcalculation_application', 'a')
        ->condition('subcalculation_id', (int) $id)
        ->countQuery()->execute()->fetchField();
      ...
      $form['overview']['sub_' . $id] = [
        ...
        'type' => ['#markup' => htmlspecialchars((string) $record['subcalculation_type'])],
    ...
    $form['add']['subcalculation_type'] = [
      ...
      '#default_value' => 'housing_type',
    ];
    $form['add']['unit_label'] = ['#type' => 'textfield', '#title' => 'Rekeneenheid', '#default_value' => 'woning'];
    $form['add']['base_quantity'] = ['#type' => 'number', '#title' => 'Basiseenheid', '#default_value' => 1, '#step' => '0.0001', '#min' => 0.0001];
    $form['add']['context_type'] = ['#type' => 'textfield', '#title' => 'Contexttype', '#description' => 'Bijvoorbeeld woningtype, technische_zone of werkpakket.'];
    $form['add']['context_ref'] = ['#type' => 'textfield', '#title' => 'Contextreferentie', '#description' => 'Verwijzing naar het canonieke gebouw-/projectobject.'];
