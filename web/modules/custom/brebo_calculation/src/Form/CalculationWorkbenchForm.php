...
final class CalculationWorkbenchForm extends FormBase {
  ...
  public function buildForm(array $form, FormStateInterface $form_state, ?NodeInterface $node = NULL): array {
    ...
    $form['workbench']['navigation']['structure'] = [
      ...
      '#url' => Url::fromRoute('brebo_calculation.structure', ['node' => $node->id()]),
      '#attributes' => ['class' => ['button']],
    ];
    $form['workbench']['navigation']['parameters'] = [
      '#type' => 'link',
      '#title' => 'Parameters & opslagen',
      '#url' => Url::fromRoute('brebo_calculation.parameters', ['node' => $node->id()]),
      '#attributes' => ['class' => ['button']],
    ];
    ...
    foreach ($structure as $key => $item) {
      ...
      foreach ($rows as $row) {
        ...
        $form['workbench']['grid']['line_' . $lineId] = [
          ...
          'operations' => ['#type' => 'link', '#title' => 'Prijsbronnen', '#url' => Url::fromRoute('brebo_calculation.price_sources', ['node' => $node->id(), 'line' => $lineId])],
        ];
      }
    }
