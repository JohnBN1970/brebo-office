...
final class CalculationPriceSourceForm extends FormBase {
  ...
  public function getFormId(): string {
    return 'brebo_calculation_price_source_form';
  }

  public function buildForm(array $form, FormStateInterface $form_state, ?NodeInterface $node = NULL, ?int $line = NULL): array {
    if (!$node instanceof NodeInterface || $node->bundle() !== 'brebo_calculation' || !$line) {
      return ['message' => ['#markup' => '<p>Calculatieregel niet gevonden.</p>']];
    }
    ...
    $row = $this->database->select('brebo_calculation_row_domain', 'r')
      ...
      ->condition('calc_line_id', $line)
      ->execute()->fetchAssoc();
    if (!$row) {
      return ['message' => ['#markup' => '<p>Deze regel hoort niet bij de actieve calculatieversie.</p>']];
    }
    ...
    $form['summary'] = [
      '#type' => 'container',
      '#attributes' => ['class' => ['brebo-price-source-summary']],
      'back' => [
        '#type' => 'link',
        '#title' => '← Terug naar calculatie',
        '#url' => Url::fromRoute('brebo_calculation.workbench', ['node' => $node->id()]),
        '#attributes' => ['class' => ['button', 'button--small']],
      ],
      'oa' => [
        '#markup' => '<div><small>Huidige OA-prijs / EH</small><strong>€ ' . number_format((float) $row['subcontracting_unit_cost'], 2, ',', '.') . '</strong></div>',
      ],
    ];

    $sources = $this->database->select('brebo_calculation_price_source_line', 'm');
    $sources->join('brebo_calculation_price_source', 's', 's.id = m.price_source_id');
    $sources->fields('m');
    $sources->fields('s', ['source_type', 'supplier_name', 'supplier_email', 'offer_number', 'offer_date', 'valid_until', 'quoted_total', 'scope_summary', 'conditions_summary', 'internal_note', 'status']);
    $sources->condition('m.calculation_id', (int) $node->id());
    $sources->condition('m.version', $version['version']);
    $sources->condition('m.calc_line_id', $line);
    $sources->orderBy('m.created', 'DESC');
    $records = $sources->execute()->fetchAllAssoc('id', \PDO::FETCH_ASSOC);

    $form['sources'] = [
      '#type' => 'table',
      '#header' => ['Bron', 'Leverancier', 'Referentie', 'Datum', 'Voorstel OA/EH', 'Status', 'Actie'],
      '#empty' => 'Nog geen prijsbronnen gekoppeld aan deze regel.',
      '#attributes' => ['class' => ['brebo-price-source-table']],
    ];

    foreach ($records as $mappingId => $source) {
      $active = (int) $source['is_active_source'] === 1;
      $approved = $source['approval_status'] === 'approved';
      $label = match ((string) $source['source_type']) {
        'email' => '✉ E-mail',
        'document' => '▣ Document',
        default => '● Handmatig',
      };
      $form['sources']['source_' . $mappingId] = [
        ...
        'status' => ['#markup' => $active ? '<strong>✓ Actieve bron</strong>' : ($approved ? 'Goedgekeurd' : 'Te controleren')],
        'action' => $active ? ['#markup' => ''] : [
          '#type' => 'submit',
          '#value' => 'Gebruik als OA-prijs',
          '#name' => 'approve_' . $mappingId,
          '#submit' => ['::approveSource'],
          '#price_source_id' => (int) $source['price_source_id'],
          '#proposed_oa_unit_cost' => (float) ($source['proposed_oa_unit_cost'] ?? 0),
          '#disabled' => $source['proposed_oa_unit_cost'] === NULL,
          '#attributes' => ['class' => ['button', 'button--small']],
        ],
      ];
    }
    ...
    $form['add'] = [
      '#type' => 'details',
      '#title' => 'Prijsbron toevoegen',
      '#open' => !$records,
      '#attributes' => ['class' => ['brebo-price-source-add']],
    ];
    $form['add']['source_type'] = [
      '#type' => 'radios',
      '#title' => 'Bronsoort',
      '#options' => ['email' => 'E-mail', 'document' => 'Document', 'manual' => 'Handmatig'],
      '#default_value' => 'email',
    ];
    ...
    $form['add']['proposed_oa_unit_cost'] = ['#type' => 'number', '#title' => 'Voorgestelde OA-prijs / EH', '#step' => '0.0001', '#min' => 0, '#required' => TRUE];
  ...
  public function submitForm(array &$form, FormStateInterface $form_state): void {
    ...
    $sourceId = $this->priceSourceManager->createForLine(
      ...
      (int) $form_state->getValue('line_id'),
      $values,
      $this->currentUser(),
    );
  ...
  public function approveSource(array &$form, FormStateInterface $form_state): void {
    ...
    $this->priceSourceManager->approveForLine(
      ...
      (string) $form_state->getValue('version'),
      (int) $form_state->getValue('line_id'),
      (int) ($trigger['#price_source_id'] ?? 0),
      (float) ($trigger['#proposed_oa_unit_cost'] ?? 0),
      'Prijsbron vanuit regelcockpit goedgekeurd.',
      $this->currentUser(),
    );
    $this->messenger()->addStatus('Prijsbron goedgekeurd en OA-prijs bijgewerkt.');
    $form_state->setRebuild(TRUE);
  }
