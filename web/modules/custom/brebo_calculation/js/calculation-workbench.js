    ...
    attach(context) {
      once('brebo-calc-workbench', '.brebo-calc-workbench__grid--editable', context).forEach((grid) => {
        ...
        const refreshClientTotals=()=>{
          ...
          structureRows.forEach((row)=>{const subtotal={direct:0,labour:0,material:0,equipment:0,subcontracting:0,other:0};childRows(row).forEach((child)=>{if(!child.classList.contains('brebo-calc-workbench__line'))return;const calc=lineTotal(child);if(!calc.included)return;subtotal.direct+=calc.total;Object.keys(calc.components).forEach((key)=>{subtotal[key]+=calc.components[key];});});const subtotalEl=row.querySelector('.brebo-calc-structure-subtotal');const serverSubtotal=Number(subtotalEl?.dataset.serverSubtotal);const canonicalDirect=Number.isFinite(serverSubtotal)?serverSubtotal:subtotal.direct;const sale=commercial(canonicalDirect);Object.keys(subtotal).forEach((key)=>setText(row,`[data-cost-kind="${key}"]`,subtotal[key]));if(subtotalEl&&!Number.isFinite(serverSubtotal))subtotalEl.textContent=money(subtotal.direct);setText(row,'[data-commercial-kind="general-cost"]',sale.generalCost);setText(row,'[data-commercial-kind="risk"]',sale.risk);setText(row,'[data-commercial-kind="margin"]',sale.margin);setText(row,'[data-commercial-kind="sales-price"]',sale.salesPrice);});
          // Grand totals are canonical server output. Do not overwrite them
          // from editable line inputs: that client subtotal excludes recipes and
          // cannot safely reconstruct effective quantities for adjustable rows.
          // Autosave/AJAX rebuild refreshes the canonical totals immediately.
        };
