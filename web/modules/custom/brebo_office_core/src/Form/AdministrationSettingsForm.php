...
final class AdministrationSettingsForm extends ConfigFormBase {
  ...
  private const SERIES = [
    'project' => 'Project', 'quotation' => 'Offerte', 'sales_invoice' => 'Verkoopfactuur',
  ...
  public function buildForm(array $form, FormStateInterface $form_state): array {
    ...
    foreach ($administrations as $code => $administration) {
      ...
      foreach ([
        'trade_name' => 'Handelsnaam', 'legal_name' => 'Statutaire naam', 'registration_number' => 'KVK / ondernemingsnummer', 'vat_number' => 'Btw-nummer',
        'address' => 'Adres', 'postal_code' => 'Postcode', 'city' => 'Plaats', 'country' => 'Landcode', 'general_phone' => 'Telefoon',
        'default_iban' => 'Standaard IBAN', 'bic' => 'BIC', 'moneybird_administration_id' => 'Moneybird administratie-ID',
      ] as $key => $label) {
        $a[$key] = ['#type' => 'textfield', '#title' => $this->t($label), '#default_value' => $administration[$key] ?? ''];
      }
  ...
  public function validateForm(array &$form, FormStateInterface $form_state): void {
    ...
    if (($new['code'] ?? '') !== '' && trim((string) ($new['trade_name'] ?? '')) === '') $form_state->setErrorByName('new_administration][trade_name', $this->t('Vul een handelsnaam in voor de nieuwe administratie.'));
  }
  ...
  public function submitForm(array &$form, FormStateInterface $form_state): void {
    ...
    foreach ($administrations as $code => &$administration) { $administration['code'] = $code; $administration['active'] = !empty($administration['active']); }
    unset($administration);
    $new = (array) $form_state->getValue('new_administration');
    $newCode = trim((string) ($new['code'] ?? ''));
    if ($newCode !== '') $administrations[$newCode] = $this->blankAdministration($newCode, trim((string) $new['trade_name']));
    $primary = (string) $form_state->getValue('primary_administration');
    if (!isset($administrations[$primary])) $primary = (string) array_key_first($administrations);
  ...
  private function legacyAdministration(Config $config): array {
    ...
    return [
      'code' => 'primary', 'active' => TRUE, 'trade_name' => (string) ($config->get('organization.trade_name') ?? 'BREBO'), 'legal_name' => (string) ($config->get('organization.legal_name') ?? 'BREBO Bouw en Advies B.V.'),
      'registration_number' => (string) ($config->get('organization.registration_number') ?? ''), 'vat_number' => (string) ($config->get('organization.vat_number') ?? ''), 'address' => (string) ($config->get('organization.address') ?? ''),
      'postal_code' => (string) ($config->get('organization.postal_code') ?? ''), 'city' => (string) ($config->get('organization.city') ?? ''), 'country' => (string) ($config->get('organization.country') ?? 'NL'),
      'general_email' => (string) ($config->get('organization.general_email') ?? ''), 'general_phone' => (string) ($config->get('organization.general_phone') ?? ''), 'website' => (string) ($config->get('organization.website') ?? ''),
      'logo_uri' => (string) ($config->get('organization.logo_uri') ?? ''), 'logo_compact_uri' => (string) ($config->get('organization.logo_compact_uri') ?? ''), 'currency' => (string) ($config->get('organization.currency') ?? 'EUR'),
      'timezone' => (string) ($config->get('project.timezone') ?? 'Europe/Amsterdam'), 'default_iban' => (string) ($config->get('organization.default_iban') ?? ''), 'bic' => (string) ($config->get('organization.bic') ?? ''),
      'moneybird_administration_id' => (string) ($config->get('organization.moneybird_administration_id') ?? ''), 'numbering' => $numbering,
    ];
  }

  private function blankAdministration(string $code, string $tradeName): array {
    $numbering = []; foreach (array_keys(self::SERIES) as $series) $numbering[$series] = $this->defaultSeries($series);
