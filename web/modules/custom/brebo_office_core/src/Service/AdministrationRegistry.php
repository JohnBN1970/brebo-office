...
final class AdministrationRegistry {
  ...
  public function primary(): array {
    return $this->get($this->primaryCode());
  }
  ...
  public function get(string $code): array {
    $all = $this->all();
    if (!isset($all[$code])) {
      throw new \InvalidArgumentException(sprintf('Onbekende administratie: %s', $code));
    }
    return $all[$code];
  }
  ...
  private function legacyPrimaryAdministration(): array {
    ...
    return [
      ...
      'numbering' => $this->defaultNumbering(),
    ];
  }

  /** @return array<string, array<string, mixed>> */
  private function defaultNumbering(): array {
    return [
      'project' => $this->series('BREBO', 1),
