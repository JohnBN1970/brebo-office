...
final class CrmFunnelWorkspaceController extends ControllerBase {
  ...
  private const MARKUP_TAGS = [
    'a', 'div', 'header', 'h2', 'p', 'progress', 'section', 'small', 'span', 'strong',
  ];
  ...
  public function overview(Request $request): array {
    ...
    try {
      $siteTimezone = new \DateTimeZone($timezoneName);
    }
    catch (\Throwable) {
    ...
    foreach ($opportunities as $opportunity) {
      ...
      if ($active && $nextActionDate !== '') {
        try {
          if (new \DateTimeImmutable($nextActionDate, $siteTimezone) < $today) {
            $overdue++;
          }
        }
        catch (\Throwable) {}
      ...
      if ($active && $opportunity->hasField('field_brebo_opp_close_date')) {
        ...
        if ($closeDate !== '') {
          try {
            $key = (new \DateTimeImmutable($closeDate, $siteTimezone))->format('Y-m');
            if (isset($forecast[$key])) {
              $forecast[$key]['count']++;
              $forecast[$key]['value'] += $value;
      ...
      $created = (new \DateTimeImmutable('@' . (int) $opportunity->getCreatedTime()))
        ->setTimezone($siteTimezone);
      $createdKey = $created->format('Y-m');
      if (isset($inflow[$createdKey])) {
        $inflow[$createdKey]['count']++;
        $inflow[$createdKey]['value'] += $value;
      }
    }
    ...
    return [
      ...
      'kpis' => [
        '#type' => 'container',
        '#attributes' => ['class' => ['brebo-crm-dashboard__kpis']],
      ...
      'charts' => [
        '#type' => 'container',
        '#attributes' => ['class' => ['brebo-crm-dashboard__charts']],
        'funnel' => [
          '#type' => 'container',
          '#attributes' => ['class' => ['brebo-crm-chart', 'brebo-crm-chart--funnel']],
          'heading' => $this->markup('<div class="brebo-crm-chart__heading"><h2>Conversietrechter</h2><p>Vaste commerciële route van marketing lead naar gewonnen. Klik op een fase om de lijst te openen.</p></div>'),
          'rows' => ['#type' => 'container', '#attributes' => ['class' => ['brebo-crm-funnel']]] + $funnelRows,
          'outflow' => $this->outflow($request, $lostCount, $lostValue, 'list'),
        ],
        'pipeline' => [
          '#type' => 'container',
          '#attributes' => ['class' => ['brebo-crm-chart', 'brebo-crm-chart--pipeline']],
          'heading' => $this->markup('<div class="brebo-crm-chart__heading"><h2>Omzetpijplijn</h2><p>Verwachte en gewogen omzet binnen dezelfde vaste commerciële trechter.</p></div>'),
          'legend' => $this->markup('<div class="brebo-crm-chart__legend"><span><i class="brebo-crm-chart__legend-total"></i> Verwachte omzet</span><span><i class="brebo-crm-chart__legend-weighted"></i> Gewogen omzet</span></div>', ['i']),
      ...
      'analytics' => [
        '#type' => 'container',
        '#attributes' => ['class' => ['brebo-crm-analytics']],
  ...
  private function applyFilters(object $query, Request $request): void {
    ...
    $statusFilter = in_array((string) $request->query->get('status', 'all'), ['all', 'open', 'closed'], TRUE)
    ...
    if (in_array($stageFilter, self::STAGES, TRUE)) {
  ...
  private function workspaceSwitch(Request $request, string $active): array {
    ...
    return [
      '#type' => 'container',
      '#attributes' => ['class' => ['brebo-crm-workspace-switch'], 'aria-label' => $this->t('CRM-weergave')],
    ] + $links;
  }

  private function kpi(string $label, string $value, string $detail): array {
    return $this->markup('<section class="brebo-crm-kpi"><span class="brebo-crm-kpi__label">' . htmlspecialchars($label, ENT_QUOTES, 'UTF-8') . '</span><strong class="brebo-crm-kpi__value">' . htmlspecialchars($value, ENT_QUOTES, 'UTF-8') . '</strong><span class="brebo-crm-kpi__detail">' . htmlspecialchars($detail, ENT_QUOTES, 'UTF-8') . '</span></section>');
  }
  ...
  private function outcomeCard(Request $request, int $wonCount, int $lostCount, float $wonValue, float $lostValue, float $winRate): array {
    ...
    $wonUrl = $this->stageUrl($request, 'Gewonnen', 'list');
    $lostUrl = $this->stageUrl($request, 'Verloren', 'list');
    return $this->markup('<section class="brebo-crm-analytics__card"><header><h2>Gewonnen versus verloren</h2><p>Actuele uitkomstverdeling van afgesloten kansen.</p></header><div class="brebo-crm-outcome"><div class="brebo-crm-outcome__rate"><strong>' . number_format($winRate, 1, ',', '.') . '%</strong><span>winrate</span></div><a href="' . htmlspecialchars($wonUrl, ENT_QUOTES, 'UTF-8') . '"><span>Gewonnen</span><progress class="is-won" max="' . $closed . '" value="' . $wonCount . '"></progress><strong>' . $wonCount . ' · ' . htmlspecialchars($this->money($wonValue), ENT_QUOTES, 'UTF-8') . '</strong></a><a href="' . htmlspecialchars($lostUrl, ENT_QUOTES, 'UTF-8') . '"><span>Verloren</span><progress class="is-lost" max="' . $closed . '" value="' . $lostCount . '"></progress><strong>' . $lostCount . ' · ' . htmlspecialchars($this->money($lostValue), ENT_QUOTES, 'UTF-8') . '</strong></a></div></section>');
  }
  ...
  private function inflowCard(array $inflow, int $max): array {
    ...
    return $this->markup('<section class="brebo-crm-analytics__card"><header><h2>Commerciële instroom · 6 maanden</h2><p>Nieuwe kansen per maand op basis van de aanmaakdatum.</p></header><div class="brebo-crm-analytics__rows">' . $rows . '</div></section>');
  }

  private function markup(string $markup, array $extraTags = []): array {
    return [
      '#markup' => $markup,
      '#allowed_tags' => array_values(array_unique([...self::MARKUP_TAGS, ...$extraTags])),
    ];
  }
