...
final class ManagementActionEngine {
  ...
  public function synchronize(int $defaultOwnerUid, ?int $now = NULL): array {
    ...
    foreach ($rules as $rule) {
      if (!$rule['trigger'] || $this->hasOpenAction($rule['key'])) {
        continue;
      }
      $id = (int) $this->database->insert('brebo_management_action')->fields([
        ...
        'status' => 'open',
        'due_at' => $now + $rule['due'],
        'context_json' => json_encode($dashboard, JSON_THROW_ON_ERROR),
        'created_at' => $now,
      ])->execute();
      $created[] = $id;
    }
  ...
  public function resolve(int $actionId, int $resolvedBy, string $resolution, ?int $now = NULL): void {
    if (trim($resolution) === '') {
      throw new \InvalidArgumentException('Een managementactie kan niet zonder inhoudelijke oplossing worden gesloten.');
    }
    $this->database->update('brebo_management_action')->fields([
      'status' => 'resolved', 'resolved_by' => $resolvedBy, 'resolution' => $resolution, 'resolved_at' => $now ?? time(),
    ])->condition('id', $actionId)->condition('status', 'open')->execute();
  }

  /** @return array<int, array<string, mixed>> */
  public function escalateOverdue(?int $now = NULL): array {
    $now ??= time();
    $rows = $this->database->select('brebo_management_action', 'a')->fields('a')->condition('status', 'open')->condition('due_at', $now, '<')->execute()->fetchAll(\PDO::FETCH_ASSOC);
    foreach ($rows as &$row) {
      $row['escalation'] = in_array($row['severity'], ['critical', 'high'], TRUE) ? 'directie' : 'management';
    }
    unset($row);
    return $rows;
  }
  ...
  private function hasOpenAction(string $key): bool {
    return (bool) $this->database->select('brebo_management_action', 'a')->condition('action_key', $key)->condition('status', 'open')->countQuery()->execute()->fetchField();
  }
}
