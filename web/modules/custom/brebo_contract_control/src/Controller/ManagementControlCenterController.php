...
final class ManagementControlCenterController extends ControllerBase {
  ...
  public function __construct(private readonly ManagementControlCenterService $controlCenter, private readonly ManagementActionEngine $actions) {}

  public static function create(ContainerInterface $container): static {
    return new static($container->get('brebo_contract_control.management_control_center'), $container->get('brebo_contract_control.management_actions'));
  }
  ...
  public function overview(): array {
    ...
    foreach ($overdue as $action) {
      $rows[] = [Link::fromTextAndUrl((string) ($action['title'] ?? ''), Url::fromRoute('brebo_contract_control.management_actions', ['action_key' => (string) ($action['action_key'] ?? 'all')]))->toRenderable(), strtoupper((string) ($action['severity'] ?? '')), (string) ($action['owner_uid'] ?? ''), !empty($action['due_at']) ? date('d-m-Y H:i', (int) $action['due_at']) : '', strtoupper((string) ($action['escalation'] ?? ''))];
    }
    ...
    foreach ($cards as $index => $card) {
      ...
      $cardItems['card_' . $index] = ['#type' => 'link', '#title' => ['#markup' => '<span class="brebo-control-kpi__label">' . htmlspecialchars($card['label']) . '</span><span class="brebo-control-kpi__value">' . htmlspecialchars($card['value']) . '</span><span class="brebo-control-kpi__hint">Bekijk acties &rarr;</span>'], '#url' => $url, '#attributes' => ['class' => ['brebo-control-kpi', 'brebo-control-kpi--' . $card['level']]]];
    }
    return [
      '#type' => 'container', '#attributes' => ['class' => ['brebo-management-control-center']], '#attached' => ['library' => ['brebo_contract_control/management_control_center']],
      'hero' => ['#type' => 'container', '#attributes' => ['class' => ['brebo-control-hero']], 'title' => ['#markup' => '<h1>Management Control Center</h1><div class="brebo-control-muted">Directiebeeld van financieel, contractueel en operationeel controlrisico.</div>'], 'status' => ['#markup' => '<div class="brebo-control-status brebo-control-status--' . $this->statusClass($managementStatus) . '">Status: ' . htmlspecialchars(str_replace('_', ' ', strtoupper($managementStatus))) . '</div>']],
      ...
      'supplier_section' => ['#type' => 'container', '#attributes' => ['class' => ['brebo-control-section']], 'title' => ['#markup' => '<h2>Leveranciersrisico</h2>'], 'table' => ['#type' => 'table', '#header' => ['Leverancier', 'TCO-score'], '#rows' => $supplierRows, '#empty' => 'Geen leveranciers onder de risicodrempel.']], '#cache' => ['max-age' => 0],
    ];
  }

  public function actions(string $action_key = 'all'): array {
    $key = $action_key === 'all' ? NULL : $action_key;
    $rows = [];
    foreach ($this->actions->openActions($key) as $action) {
      $rows[] = [(int) $action['id'], (string) $action['title'], strtoupper((string) $action['severity']), (string) $action['status'], (string) $action['owner_uid'], date('d-m-Y H:i', (int) $action['due_at']), (string) $action['source_type']];
    }
    return ['#type' => 'container', '#attached' => ['library' => ['brebo_contract_control/management_control_center']], 'back' => Link::fromTextAndUrl('← Terug naar Control Center', Url::fromRoute('brebo_contract_control.management_control_center'))->toRenderable(), 'title' => ['#markup' => '<h1>Managementacties</h1><p>Filter: <strong>' . htmlspecialchars($action_key) . '</strong></p>'], 'table' => ['#type' => 'table', '#header' => ['ID', 'Actie', 'Ernst', 'Status', 'Eigenaar', 'Deadline', 'Bron'], '#rows' => $rows, '#empty' => 'Geen open acties voor dit signaal.'], '#cache' => ['max-age' => 0]];
  }
