...
final class PurchaseInvoiceIntegrationClient {
  ...
  public function fetchAll(): array {
    ...
    if ($status < 200 || $status >= 300 || !is_array($decoded) || ($decoded['status'] ?? NULL) !== 'ok') {
      ...
      throw new \RuntimeException(sprintf('BREBO integration API rejected purchase invoice read (%d, %s).', $status, $code));
    }

    $invoices = $decoded['purchase_invoices'] ?? NULL;
    if (!is_array($invoices)) {
      throw new \RuntimeException('BREBO integration API returned an invalid purchase invoice payload.');
    }
