...
final class InvoiceBlockerActionManager {
  ...
  public function forOwner(int $ownerUid): array {
    $this->ensureStorage(); $now=time(); $today=(new \DateTimeImmutable('today'))->getTimestamp(); $tomorrow=(new \DateTimeImmutable('tomorrow'))->getTimestamp();
    $q=$this->database->select('brebo_finance_invoice_blocker_action','a')->fields('a')->condition('owner_uid',$ownerUid)->condition('status',['open','in_progress','waiting'],'IN')->orderBy('due_date','ASC')->orderBy('changed','DESC');
    $items=array_map(static fn(object $r):array=>(array)$r,$q->execute()->fetchAll());
    foreach($items as &$item){$due=$item['due_date']===null?null:(int)$item['due_date'];$item['urgency']=$due===null?'no_deadline':($due<$today?'overdue':($due<$tomorrow?'today':'upcoming'));$item['days_overdue']=$due!==null&&$due<$today?(int)floor(($today-$due)/86400):0;}
    unset($item); return $items;
  }

  public function ownerSummary(int $ownerUid): array {
    $items=$this->forOwner($ownerUid);$summary=['total'=>count($items),'overdue'=>0,'today'=>0,'upcoming'=>0,'no_deadline'=>0,'open'=>0,'in_progress'=>0,'waiting'=>0];
    foreach($items as $item){$summary[$item['urgency']]++;$summary[$item['status']]++;}return ['summary'=>$summary,'items'=>$items];
  }
