...
final class InvoiceBlockerActionManager {
  ...
  public function forOwner(int $ownerUid): array {
    ...
    foreach($items as &$item){$due=$item['due_date']===null?null:(int)$item['due_date'];$item['urgency']=$due===null?'no_deadline':($due<$today?'overdue':($due<$tomorrow?'today':'upcoming'));$item['days_overdue']=$due!==null&&$due<$today?(int)floor(($today-$due)/86400):0;}unset($item);return $items;
  }

  public function ownerSummary(int $ownerUid): array {$items=$this->forOwner($ownerUid);$summary=['total'=>count($items),'overdue'=>0,'today'=>0,'upcoming'=>0,'no_deadline'=>0,'open'=>0,'in_progress'=>0,'waiting'=>0];foreach($items as $item){$summary[$item['urgency']]++;$summary[$item['status']]++;}return ['summary'=>$summary,'items'=>$items];}
