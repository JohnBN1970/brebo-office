...
final class FinancialProjectLedger {
  ...
  public function build(int $projectNid): array {
    return [
      ...
      'purchase_invoice_lines' => $this->purchaseInvoiceLines($projectNid),
  ...
  private function purchaseInvoices(int $projectNid): array {
    return $this->rows('brebo_finance_purchase_invoice',$projectNid,['id','supplier_name','invoice_number','invoice_date','due_date','status','match_status','amount_ex_vat','vat_amount','amount_inc_vat','created','changed']);
  }
  ...
  private function commitmentLines(int $projectNid): array {
    ...
    $q=$this->database->select('brebo_finance_commitment_line','cl'); $q->join('brebo_finance_commitment','c','c.id = cl.commitment_id');
    $q->addField('cl','id'); $q->addField('cl','commitment_id'); $q->addField('c','commitment_number'); $q->addField('c','supplier_name');
    foreach(['description','amount_ex_vat','unit_price_ex_vat','quantity'] as $field) if($schema->fieldExists('brebo_finance_commitment_line',$field)) $q->addField('cl',$field);
    $q->condition('c.project_nid',$projectNid)->orderBy('cl.id','ASC'); return array_map(static fn(object $r):array=>(array)$r,$q->execute()->fetchAll());
  }

  private function rows(string $table, int $projectNid, array $wanted): array {
    $schema = $this->database->schema(); if (!$schema->tableExists($table)) return [];
    $fields = array_values(array_filter($wanted, static fn(string $field): bool => $schema->fieldExists($table, $field))); if ($fields === [] || !$schema->fieldExists($table, 'project_nid')) return [];
    $query = $this->database->select($table, 'x')->fields('x', $fields)->condition('project_nid', $projectNid);
    if ($schema->fieldExists($table, 'changed')) $query->orderBy('changed', 'DESC'); elseif ($schema->fieldExists($table, 'created')) $query->orderBy('created', 'DESC');
  ...
  private function audit(int $projectNid): array {
    ...
    $query = $this->database->select('brebo_finance_audit', 'a')->fields('a')->condition('project_nid', $projectNid)->orderBy('created', 'DESC')->range(0, 100);
    return array_map(static fn(object $row): array => (array) $row, $query->execute()->fetchAll());
  }
}
