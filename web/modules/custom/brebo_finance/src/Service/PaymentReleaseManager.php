...
final class PaymentReleaseManager {
  ...
  public function prepare(int $invoiceId, string $releaseNumber, string $requestedPaymentDate, int $userId): int {
    ...
    if (in_array($invoice['status'], ['paid', 'cancelled'], TRUE)) {
      throw new RuntimeException('A paid or cancelled invoice cannot be released.');
    }

    $regular = (string) $invoice['regular_account_amount'];
    $gAccount = (string) $invoice['g_account_amount'];
    $splitTotal = $this->decimal->add($regular, $gAccount);
    if ($this->decimal->compare($splitTotal, (string) $invoice['amount_inc_vat']) !== 0) {
      throw new RuntimeException('Regular-account and G-account amounts do not equal the invoice total.');
    }
    if ($this->decimal->compare($gAccount, '0') > 0) {
      $this->assertApprovedGAccountInstruction($invoiceId, $gAccount);
    }
