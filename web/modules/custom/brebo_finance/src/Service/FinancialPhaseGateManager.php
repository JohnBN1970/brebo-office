...
final class FinancialPhaseGateManager {
  ...
  /**
   * Returns a deterministic release decision. AI can never override it.
   *
  ...
  /**
   * Requests a temporary, evidence-backed exception; this never releases yet.
   */
  public function requestException(
    ...
    $blockers = $this->loadBlockingFindings($projectNid);
    $knownIds = array_map(static fn(array $row): int => (int) $row['id'], $blockers);
    ...
    $id = (int) $this->database->insert('brebo_finance_phase_gate_exception')
      ->fields([
        'project_nid' => $projectNid,
        'gate' => $gate,
        'status' => 'requested',
  ...
  /**
   * Applies four-eyes approval or rejection to a requested exception.
   */
  ...
  /**
   * Throws when a protected workflow transition is not released.
   */
