...
final class StandaloneSalesInvoiceReleaseForm extends ConfirmFormBase {
  ...
  public function getFormId(): string { return 'brebo_finance_standalone_sales_invoice_release_form'; }
  ...
  public function getCancelUrl(): Url { return Url::fromRoute('brebo_finance.sales_workspace'); }
  ...
  public function buildForm(array $form, FormStateInterface $form_state, ?int $draft = NULL): array {
    ...
    if (!empty($context['g_account_on'])) {
      $gross = round((float) $row['amount_inc_vat'], 2); $regular = round((float) ($context['regular_account_amount'] ?? 0), 2); $g = round((float) ($context['g_account_amount'] ?? 0), 2);
      if (abs(($regular + $g) - $gross) > 0.01) throw new \RuntimeException('G-rekeningverdeling sluit niet aan op het factuurtotaal. Open het concept en sla het opnieuw op.');
      if (trim((string) ($context['g_account_iban'] ?? '')) === '') throw new \RuntimeException('G-rekening IBAN ontbreekt. Configureer dit eerst in Verkoopinstellingen.');
    }
    ...
    $form_state->set('draft_id', $draftId); $form_state->set('moneybird_contact_id', $moneybirdContactId);
  ...
  public function submitForm(array &$form, FormStateInterface $form_state): void {
    ...
    if ($invoiceNumber === '') {
      $invoiceYear = (int) substr((string) $draft['invoice_date'], 0, 4); $invoiceNumber = $this->numberManager->reserveNextAutomatic($invoiceYear > 0 ? $invoiceYear : NULL);
      $context['final_invoice_number'] = $invoiceNumber; $context['final_number_reserved_at'] = time(); $this->saveContext($draftId, $context);
    }

    $pdf = $this->outputBuilder->finalPdf($draftId, $invoiceNumber);
    $selected = array_values(array_filter(array_map('intval', (array) $form_state->getValue('document_attachments', [])))); $extraAttachments = $this->attachmentService->resolveDocumentIds($selected);
    ...
    if (empty($mail['result'])) { $this->messenger()->addError($this->t('De definitieve factuur kon niet worden verzonden. Nummer @number blijft gereserveerd.', ['@number' => $invoiceNumber])); return; }

    $this->numberManager->markUsed($invoiceNumber); $now = time();
    $context['final_invoice_number'] = $invoiceNumber; $context['final_pdf_hash'] = $pdf['hash']; $context['final_pdf_filename'] = $pdf['filename']; $context['final_sent_at'] = $now; $context['final_recipient'] = $recipient; $context['final_attachment_document_ids'] = $selected;
  ...
  private function buildPayload(array $draft, array $lines, string $moneybirdContactId, array $context, string $invoiceNumber): array {
    return ['schema' => 'brebo.sales_invoice.register.v1', 'source' => ['system' => 'brebo-office', 'draft_id' => (int) $draft['id'], 'draft_number' => (string) $draft['draft_number'], 'invoice_number' => $invoiceNumber, 'project_nid' => 0], 'invoice' => [
      'invoice_id' => $invoiceNumber, 'contact_id' => $moneybirdContactId, 'reference' => trim((string) ($context['customer_ref'] ?? '')), 'invoice_date' => (string) $draft['invoice_date'], 'due_date' => (string) $draft['due_date'], 'description' => (string) ($draft['description'] ?? ''),
      'amount_ex_vat' => (string) $draft['amount_ex_vat'], 'vat_amount' => (string) $draft['vat_amount'], 'amount_inc_vat' => (string) $draft['amount_inc_vat'],
