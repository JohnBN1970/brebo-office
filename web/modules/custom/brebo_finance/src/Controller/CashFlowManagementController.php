...
final class CashFlowManagementController extends ControllerBase {
  ...
  public function page(): array {
    ...
    foreach ($forecast['weeks'] as $week) {
      $weekRows[] = ['Week ' . $week['week'], $week['start_date'] . ' t/m ' . $week['end_date'], $this->money((float) $week['incoming']), $this->money((float) $week['outgoing']), $this->money((float) $week['net']), $this->money((float) $week['g_account_incoming']), $this->money((float) $week['g_account_outgoing'])];
    }

    $signalItems = '';
    foreach ($report['signals'] as $signal) $signalItems .= '<li>' . htmlspecialchars((string) $signal, ENT_QUOTES, 'UTF-8') . '</li>';
    foreach (($liquidity['reasons'] ?? []) as $reason) $signalItems .= '<li>' . htmlspecialchars((string) $reason, ENT_QUOTES, 'UTF-8') . '</li>';
    ...
    if (($health['status'] ?? 'unavailable') === 'ok') {
      ...
      if (($health['break_even']['monthly_revenue_required'] ?? NULL) !== NULL) $healthMarkup .= '<p>Benodigde maandelijkse break-evenomzet: <strong>' . $this->money((float) $health['break_even']['monthly_revenue_required']) . '</strong>.</p>';
    }
    else $healthMarkup .= '<p>Moneybird bedrijfsgezondheid is tijdelijk niet beschikbaar; de operationele Office-data hieronder blijft bruikbaar.</p>';
    ...
    if ($projectedRegular !== NULL && $projectedG !== NULL) $liquidityMarkup .= '<p>Verwacht na 90 dagen: regulier <strong>' . $this->money($projectedRegular) . '</strong> · G-rekening <strong>' . $this->money($projectedG) . '</strong>.</p>';
    $liquidityMarkup .= '</section>';

    $overdueRows = [];
    foreach ($report['overdue_invoices'] as $invoice) {
      $overdueRows[] = [
        ['data' => ['#type' => 'link', '#title' => $invoice['invoice_number'], '#url' => Url::fromRoute('brebo_finance.receivables_action', ['invoice' => $invoice['id']])]],
        $invoice['project_nid'] > 0 ? (string) $invoice['project_nid'] : 'Los',
        $invoice['due_date'],
        $invoice['days_overdue'] . ' dagen',
    ...
    foreach ($report['projects'] as $project) {
      $projectRows[] = [$project['project_nid'] > 0 ? (string) $project['project_nid'] : 'Los', $this->money((float) $project['invoiced']), $this->money((float) $project['received']), $this->money((float) $project['outstanding']), $this->money((float) $project['overdue'])];
    }
    ...
    return [
      ...
      'kpis' => ['#markup' => '<div class="bfcc-grid">' . $this->card('Banksaldo regulier', $openingRegular) . $this->card('G-rekening', $openingG) . $this->card('Gefactureerd', (float) $sales['invoiced']) . $this->card('Ontvangen', (float) $sales['received']) . $this->card('Openstaand', (float) $sales['outstanding']) . $this->card('Vervallen', (float) $sales['overdue']) . $this->card('13w inkomend', (float) $forecast['incoming_13w']) . $this->card('13w uitgaand', (float) $forecast['outgoing_13w']) . $this->card('13w netto beweging', (float) $forecast['net_13w']) . '</div>'],
      ...
      'weeks' => ['#type' => 'table', '#caption' => $this->t('Controller · 13-weeks cashflowprognose'), '#header' => [$this->t('Week'), $this->t('Periode'), $this->t('Inkomend'), $this->t('Uitgaand'), $this->t('Netto'), $this->t('G-rekening in'), $this->t('G-rekening uit')], '#rows' => $weekRows],
      ...
      'overdue' => ['#type' => 'table', '#caption' => $this->t('Controller · grootste vervallen posten'), '#header' => [$this->t('Factuur'), $this->t('Project'), $this->t('Vervaldatum'), $this->t('Achterstand'), $this->t('Openstaand'), $this->t('Status')], '#rows' => $overdueRows, '#empty' => $this->t('Geen vervallen openstaande facturen.')],
      'projects' => ['#type' => 'table', '#caption' => $this->t('Controller · debiteuren per project'), '#header' => [$this->t('Project'), $this->t('Gefactureerd'), $this->t('Ontvangen'), $this->t('Openstaand'), $this->t('Vervallen')], '#rows' => $projectRows, '#empty' => $this->t('Geen projectdebiteuren beschikbaar.')],
  ...
  private function card(string $label, float $amount): string { return '<article class="bfcc-card"><span>' . htmlspecialchars($label, ENT_QUOTES, 'UTF-8') . '</span><strong>' . $this->money($amount) . '</strong></article>'; }
