(function (Drupal, once) {
  ...
  Drupal.behaviors.breboFinanceProjectDetail={attach(context){once('bfpd','#brebo-finance-project-detail',context).forEach(async(app)=>{
    ...
    try{
      ...
      const show=(key)=>{const t=tabs.find(x=>x[0]===key);nav.querySelectorAll('button').forEach(b=>b.classList.toggle('is-active',b.dataset.tab===key));panel.innerHTML=table(t[2]||[],t[3],t[4]);bindTrace();};
