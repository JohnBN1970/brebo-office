    ...
    attach(context) {
      once('brebo-finance-decisions', '#brebo-finance-decision-app', context).forEach((app) => {
        ...
        const decide = async (id, decision) => {
          ...
          if (note === null) return;
          const token = await csrf();
          const response = await fetch(`${app.dataset.decisionBaseUrl}/${id}/decision`, {
        ...
        const loadInbox = async () => {
          ...
          try {
            ...
            status.textContent = data.count ? `${data.count} besluit${data.count === 1 ? '' : 'en'} wacht${data.count === 1 ? '' : 'en'} op jou` : 'Geen financiële besluiten die op jou wachten.';
            list.innerHTML = data.items.length ? data.items.map((item) => {
              ...
              return `<article class="bfd-card" data-exception-id="${item.exception_id}">
                <div class="bfd-card-top"><div><span class="bfd-chip">${esc(gateLabel(item.gate))}</span><h2>Project #${item.project_nid}</h2></div><div class="bfd-money">${money(exposure)}</div></div>
            ...
            list.querySelectorAll('[data-decision]').forEach((button) => button.addEventListener('click', async () => {
              button.disabled = true;
              try { await decide(button.closest('[data-exception-id]').dataset.exceptionId, button.dataset.decision); }
              catch (error) { window.alert(error.message); button.disabled = false; }
            }));
          } catch (error) {
            status.textContent = 'Beslisinbox kon niet worden geladen.';
            list.innerHTML = `<div class="bfd-error">${esc(error.message)}</div>`;
          }
        };
