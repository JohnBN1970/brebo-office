...
/** Creates canonical CRM opportunities for external project requests. */
final class ExternalProjectRequestIntakeDestination implements IntakeDestinationInterface {
  ...
  public function supports(string $classification): bool {
    return in_array($classification, ['external_project_request', 'website_project_request'], TRUE);
  }
  ...
  public function route(array $envelope): IntakeDestinationResult {
    ...
    $source = trim((string) ($payload['source'] ?? $metadata['source'] ?? 'website'));
    $sourceLabel = trim((string) ($metadata['source_label'] ?? ''));
    if ($sourceLabel === '') {
      $sourceLabel = $source === 'brebo-platform.europakozijn' ? 'Website - Europakozijn' : $source;
    }
    ...
    $title = sprintf('%s - %s [%s]', $sourceLabel, $label, substr($requestId, 0, 8));
    ...
    if ($existingIds !== []) {
      ...
      if ($existing instanceof NodeInterface) {
        return new IntakeDestinationResult(
          IntakeDestinationResult::REVIEW_REQUIRED,
          'external_project_request_requires_review',
    ...
    $values = [
      ...
      'field_brebo_opp_source' => mb_substr($sourceLabel, 0, 255),
