export class MoneybirdResponseError extends Error {
  constructor(readonly status: number, message = "Moneybird request failed.") { super(message); this.name = "MoneybirdResponseError"; }
}
export interface SalesInvoiceDispatch { idempotency_key: string; sales_invoice: Record<string, unknown>; sending?: Record<string, unknown>; }
export interface MoneybirdSalesInvoiceResult { id: string; invoice_id: string | null; state: string; invoice_date: string | null; due_date: string | null; sent_at: string | null; }
export interface MoneybirdSalesInvoiceReceivableResult extends MoneybirdSalesInvoiceResult { contact_id: string | null; currency: string; total_price_excl_tax: string | null; total_price_incl_tax: string | null; paid_amount: string; version: string | number | null; }
...
export async function checkMoneybirdConnection(env: Env): Promise<MoneybirdConnectionResult> {
  ...
  const administration=value.find(candidate=>!!candidate&&typeof candidate==="object"&&String((candidate as Record<string,unknown>).id??"")===administrationId);
  if(!administration||typeof administration!=="object") throw new MoneybirdResponseError(404,"Configured Moneybird administration is not accessible.");
  return {administration_id:administrationId,administration_name:typeof (administration as Record<string,unknown>).name==="string"?(administration as Record<string,unknown>).name as string:null};
}

export async function listPurchaseInvoices(env: Env): Promise<MoneybirdPurchaseInvoiceResult[]> {
  const base=`https://moneybird.com/api/v2/${encodeURIComponent(env.MONEYBIRD_ADMINISTRATION_ID)}`, result:MoneybirdPurchaseInvoiceResult[]=[]; const perPage=100;
  for(let page=1;page<=100;page++){const response=await moneybirdFetch(`${base}/documents/purchase_invoices.json?per_page=${perPage}&page=${page}&filter=${encodeURIComponent("period:this_year,state:all")}`,env,{method:"GET"}); const value:unknown=await response.json(); if(!Array.isArray(value)) throw new MoneybirdResponseError(502,"Invalid Moneybird purchase invoice response."); for(const candidate of value) result.push(purchaseInvoiceResult(candidate)); if(value.length<perPage) break;} return result;
}
