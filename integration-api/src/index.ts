...
export default {
  async fetch(request: Request, env: Env): Promise<Response> {
    ...
    let requestId = request.headers.get("X-BREBO-Request-Id") ?? crypto.randomUUID();
...
async function analyze(request: Request, env: Env, path: string, requestId: string): Promise<Response> {
  ...
  if (!parsed.success) {
    ...
    return errorResponse(
      unsafeTestData ? 422 : 400,
      unsafeTestData ? "unsafe_or_unprocessable" : "invalid_request",
      unsafeTestData ? "Test mode cannot process real data." : "Request does not match the contract.",
      requestId,
    );
  }

  if (parsed.data.communication.message.length > numberSetting(env.MAX_INPUT_CHARS, 12_000)) {
    return errorResponse(400, "invalid_request", "Message exceeds the configured limit.", requestId);
  }

  const requestHash = await sha256Hex(requestId);
  const replay = env.REPLAY_GUARD.getByName(requestHash.slice(0, 2));
  const now = Math.floor(Date.now() / 1_000);
  const accepted = await replay.useOnce(
    requestHash,
    now + numberSetting(env.REPLAY_TTL_SECONDS, 600),
    now,
  );
  if (!accepted) {
  ...
  try {
    ...
    return Response.json({
      status: "ok",
      mode: parsed.data.test_mode ? "test" : "production",
      stored: false,
      sent: false,
    ...
    if (error instanceof ProviderResponseError) {
      return errorResponse(502, "provider_error", "AI provider could not complete the analysis.", requestId);
    }
    throw error;
  }
}
